    ...
    const likeEvent = async(e) => {
        if(type){
            if(islike){
                ...
                let data = await fetch(`http://localhost:1800/posts/add/${props.obj.obj.postid}`, requestOptions)
                ...
                let data = await fetch(`http://localhost:1800/posts/s/${props.obj.obj.postid}`, requestOptions)
                    .then(response => response.json())
                    .then(data => {
                        setLike(true)
                        setLikes(data.likes)
